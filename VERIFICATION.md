...
| Reduced motion | `index.html`, `proof.html` | final state shown immediately; 0 running animations; Run reveals all eight steps |
...
| Proof replay determinism | `node --test tests/proof.test.cjs` replay harness | trace hash `3cf4a4c3`; the timed run and the reduced-motion run agree |
| Recipe determinism | `recipes/gateway-lease-fence`, `cargo run --locked --release` | three warm runs: all `68f8180b2110b92bdcc283bcbbaf4461bda4ba66e9a7bce78151b6409dcdc769`, equal to `fixture.json` (LF) and to the fixture embedded in `proof.html` |
